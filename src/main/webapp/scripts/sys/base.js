...
function showTree2(obj,dept_ID){
	var tree = XTreeFactory.makeTree(obj.id+"_tree", "人员选择", function() {
		var nodes = tree.getSelectedNodes();
		var name_ = obj.id.replace("_FK","Name");
...
function validate(action) {
	if(action==1){
		if (typeof (validateForm1) != "undefined") {
			var x = validateForm1(action);
			if (x) {
				if (x != "-1") {
					alert(x);
				}
...
function menubar_onclick(x) {
	...
	if (x == 0 || x == 1 || x == 2 || x == 9) {
		...
		var sid = document.getElementById("orderNumber").value.substr(0 , 3);
		if(sid == "ACR"){
			if(x == 6 && !validate(6)){
				return;
			}
...
function initdatepicker3(x) {
	$("#" + x).datePicker({
		...
		onClose: function(dateText, inst) {
			...
			$("#" + x).val(year+'-'+(parseInt(month)+1));//给input赋值，其中要对月值加1才是实际的月份
		}
	});			
}

function initdatepicker2(x) {
	$("#" + x).datepicker({
	     changeMonth: true,
	     changeYear: true,
	     dateFormat: 'yy-MM',
